...
} from "./commandConsumer";

type TestCommand = ConsumerCommand & { payload: string };

function cmd(id: string, createdAt: number): TestCommand {
...
// The consumer is agnostic to the actual convex function names — callers pass
// their own refs (commands:* for the gateway, hosts:* for the hostagent).
const refs = {
  pendingFor: makeFunctionReference<"query">("q:pendingFor"),
  claim: makeFunctionReference<"mutation">("q:claim"),
  ack: makeFunctionReference<"mutation">("q:ack"),
  heartbeat: makeFunctionReference<"mutation">("q:heartbeat"),
};
...
function startConsumer(options: {
  ...
  return startCommandConsumer<TestCommand>({
    client: options.client,
    refs,
    secret: "s",
    subscriptionArgs: { consumerId: "consumer-1" },
    buildHeartbeatArgs: () => ({ consumerId: "consumer-1" }),
    logPrefix: "test",
    execute: options.execute,
    ...(options.claimRetryDelaysMs !== undefined
      ? { claimRetryDelaysMs: options.claimRetryDelaysMs }
      : {}),
  });
}
...
function fakeClient() {
  ...
  let subscriber: ((commands: TestCommand[]) => void) | undefined;
  const client = {
    onUpdate: (
      _ref: unknown,
      _args: unknown,
      callback: (commands: TestCommand[]) => void,
    ) => {
      subscriber = callback;
      return () => {};
    },
    mutation: async (ref: unknown, args: Record<string, unknown>) => {
      ...
      return name === "q:claim" ? true : undefined;
    },
    close: async () => {},
  } as unknown as ConvexClient;
  const push = (commands: TestCommand[]) => {
    subscriber?.(commands);
  };
...
function fakeServer(initial: TestCommand[]) {
  type Row = { cmd: TestCommand; status: "pending" | "running" | "acked" };
  const rows = new Map<string, Row>(
    initial.map((c) => [c.commandId, { cmd: c, status: "pending" }]),
  );
  const subscribers = new Set<(commands: TestCommand[]) => void>();
  const pending = () =>
    [...rows.values()]
...
test("executes serially, acks even on failure, ignores re-delivery", async () => {
  ...
  // Both acked despite "b" throwing; heartbeat fired on start with the
  // caller-built args plus the secret.
  const acks = mutations.filter((m) => m.name === "q:ack");
  expect(acks.map((m) => m.args.commandId)).toEqual(["a", "b"]);
  expect(acks.every((m) => m.args.secret === "s")).toBe(true);
  const heartbeats = mutations.filter((m) => m.name === "q:heartbeat");
...
// and the ack must not re-execute the command after restart. Side effects are
// not idempotent (a replayed `start` could evict a live session; a replayed
// provision_vm double-allocates a VM) — the claim lifecycle prevents it.
...
function flakyClaimClient(failClaims: number, claimResult = true) {
  ...
  const push = (commands: TestCommand[]) => subscriber?.(commands);
