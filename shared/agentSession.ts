...
import { createRingBuffer, type RingBuffer } from "./agentHistory";
import {
  clip,
  excerpt,
...
// prompt, the event sender/uploader (each host's own endpoint + secret), the
// per-task MCP servers, the optional screen recorder (devbox only — local
// machines are never recorded, per privacy), and the optional tool gate
// (localagent's hard bans).

/**
 * The slice of the SDK `Query` interface the session depends on. Tests stub
...
/** The recorder slice the session drives (gateway/src/recorder.ts satisfies
 * it). The localagent passes none: no recording of the user's machine. */
...
/**
 * Pre-flight veto over a tool call (#138: the localagent's Codex-style hard
 * bans — terminals, admin auth, OS security prompts). Runs for every tool
 * except AskUserQuestion (which blocks on a human answer instead). Return a
 * PermissionResult to decide the call, or null to fall through to allow.
 */
export type ToolGate = (
  toolName: string,
  input: Record<string, unknown>,
) => Promise<PermissionResult | null> | PermissionResult | null;

export type AgentSessionDeps = {
  emitEvent: EventSender;
  /** The session's only standing instruction (the SDK default is an empty
   * system prompt); the per-task spec rides in as the first user message. */
  systemPrompt: string;
  /** Log prefix, e.g. "gateway" or "localagent". */
  logPrefix: string;
  /** Pre-flight veto over tool calls (see ToolGate). */
  toolGate?: ToolGate;
...
/**
 * Owns a host's single live Agent SDK session (one per devbox gateway; one
 * per local machine).
 *
...
export class AgentSessionManager {
  #deps: Required<Pick<AgentSessionDeps, "emitEvent" | "now">> &
    Pick<AgentSessionDeps, "onMessage" | "onStatusChange">;
  #queryFn: QueryFn;
  #createMcpServers:
  ...
  constructor(deps: AgentSessionDeps) {
    ...
    this.#toolGate = deps.toolGate ?? null;
    this.#deps = {
  ...
  start(request: StartTaskRequest): boolean {
    ...
    const options: Options = {
      ...
      canUseTool: async (toolName, input) => {
        ...
        if (this.#toolGate !== null) {
          const verdict = await this.#toolGate(toolName, input);
      ...
      stderr: (data: string) => {
        ...
        if (line.length > 0)
          console.error(`[${this.#logPrefix}] sdk-stderr:`, line);
      },
  ...
  #checkWatchdog(taskId: string): void {
    ...
    console.error(
      `[${this.#logPrefix}] session watchdog tripped for ${taskId}: ${reason}`,
    );
    // One trip is final.
    ...
    const hardExit = setTimeout(() => {
      if (this.#running) {
        console.error(
          `[${this.#logPrefix}] watchdog: session did not wind down 30s after interrupt; exiting for a clean relaunch`,
        );
        process.exit(1);
      }
    }, 30_000);
  ...
  async stop(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[${this.#logPrefix}] interrupt failed:`, error);
    }
    this.#queue.end();
    return true;
  }
  ...
  async #run(query: AgentQuery, taskId: string): Promise<void> {
    try {
      for await (const message of query) {
        ...
        console.log(`[${this.#logPrefix}] sdk message: ${message.type}`);
        this.#history.push(message);
        this.#deps.onMessage?.(message);
        this.#handleLifecycle(message);
      }
    } catch (error) {
      console.error(`[${this.#logPrefix}] session error:`, error);
  ...
  async #emitToolResults(
    ...
    for (const result of extractToolResults(message)) {
      ...
      try {
        ...
        console.error(
          `[${this.#logPrefix}] tool-result emission failed:`,
          error,
        );
      }
    }
